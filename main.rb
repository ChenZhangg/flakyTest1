...
def create_github_repo(repo_dir,user_name,repo_name,i)
  ...
  while(i<600)
    ...
    if $?.to_i!=0
      ...
      puts "*******************Create repo #{@github_username}/#{user_name}_#{repo_name}_#{i} on Github successfully.**************************"
      break
    end
  end
end
...
def open_travis(repo_dir,user_name,repo_name,i)
  k=0
  while(k<600)
    %x"cd #{repo_dir} && travis enable -r #{@github_username}/#{user_name}_#{repo_name}_#{i}"
    if $?.to_i!=0
      puts "*******************The #{k} of 600 try to enable #{@github_username}/#{user_name}_#{repo_name}_#{i}. $?=#{$?.to_i}*******************************"
      k+=1
      sleep 60
      next
    else
      puts "*******************Enable travis #{@github_username}/#{user_name}_#{repo_name}_#{i} successfully.**************************"
      break
    end
  end
end
...
def link_remote(repo_dir,user_name,repo_name,i)
  k=0
  while(k<600)
    %x"cd #{repo_dir} && git remote add #{user_name}_#{repo_name}_#{i} git@github.com:#{@github_username}/#{user_name}_#{repo_name}_#{i}.git"
    if $?.to_i==0
      puts "*******************Remote add #{@github_username}/#{user_name}_#{repo_name}_#{i} successfully.**************************"
      break
    elsif $?.to_i==128
      puts "*******************Already add #{@github_username}/#{user_name}_#{repo_name}_#{i}.**************************"
      break
    else
      puts "*******************The #{k} of 600 try to remote add #{@github_username}/#{user_name}_#{repo_name}_#{i}. $?=#{$?.to_i}*******************************"
      k+=1
      sleep 60
      next
    end
...
def get_last_build(user_name,repo_name,i)
  ...
  begin
    @client.clear_cache!
    travis_repo= @client.repo("#{@github_username}/#{user_name}_#{repo_name}_#{i}")
    last=travis_repo.last_build
  rescue
    puts $!
    travis_repo=nil
    last=nil
    puts "*******************The #{k} of 10 try to get last build of #{@github_username}/#{user_name}_#{repo_name}_#{i}.**************************"
    sleep 60
    k+=1
    retry if k<10
  end
...
def pre_build_completed(user_name,repo_name,i)
  ...
  puts "*******************The information of last build of #{@github_username}/#{user_name}_#{repo_name}_#{i} is #{last ? last.number : '0'}********************"
  while(last && last.finished? != true)
    puts "*******************The last build #{last ? last.number : '0'} of #{@github_username}/#{user_name}_#{repo_name}_#{i} is not finished.********************"
    sleep 300
    last=get_last_build(user_name,repo_name,i)
  end
end
...
def push(repo_dir,user_name,repo_name,l,i)
  ...
  while(k<5)
    ...
    if $?.to_i==0
      puts "*******************Push #{l.sha}  #{l.date} to Github repo #{@github_username}/#{user_name}_#{repo_name}_#{i}*******************"
      break
    elsif $?.to_i==256
      puts "*******************Push Error $?=#{$?} #{l.sha}  #{l.date} to Github repo #{@github_username}/#{user_name}_#{repo_name}_#{i}*******************"
      k+=1
      sleep 60
      next
    elsif $?.to_i==32768
      puts "*******************Push Error $?=#{$?.to_i} #{l.sha}  #{l.date} to Github repo #{@github_username}/#{user_name}_#{repo_name}_#{i}*******************"
      k+=1
      sleep 60
      next
    else
