...
def pre_build_completed(user_name,repo_name,i)
  ...
  puts "*******************The information of last build of ChenZhangg/#{user_name}_#{repo_name}_#{i} is #{last}********************"
  while(last && last.finished? != true)
    puts "*******************The last build #{last} of ChenZhangg/#{user_name}_#{repo_name}_#{i} is not finished.********************"
    sleep 300
    last=get_last_build(user_name,repo_name,i)
  end
end
